...
class Pictures extends Component{
    renderPictures =()=>{
        ...
        if(arr.length >0){
          ...
          return(<Col data-ca="ca" style={{backgroundImage : `url(${imgs[img.replace(/-/g, '').toLowerCase()]})`}} className={`tag-img tags`} md="6">
           <Link to={`/dashboard/${img.toLowerCase()}`} style={{ textDecoration: "none" }}>
              <div className="cover ">
                {" "}
                <p>{img.replace(/-/g, ' ')}</p>
              </div></Link>
